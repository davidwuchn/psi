...
- Verification: `emacs -Q --batch -L components/emacs-ui -l components/emacs-ui/test/psi-test.el -f ert-run-tests-batch-and-exit` → green after compose follow-up updates.
...
- OpenAI provider contract distillation landed (`6e39269`): `spec/openai-provider.allium` now captures provider dispatch, request/auth requirements, SSE normalization, tool-call streaming edge cases, and request/reply capture callbacks; next slice should add `traces:` coverage and tighten canonical reason mapping against `ai-abstract-model.allium` if needed.
- Remaining work:
