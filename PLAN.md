...
- Emacs footer-preservation across transcript reset landed (`e91c490`): `psi-session-commands.el` saves `projection-footer` before `reset-transcript-state` and restores it after for both `/new` and `switch_session` success paths, preventing footer flicker when events arrive before the response frame.
