...
- `psi-emacs--session-display-name` now accepts both payload vocabularies:
  - event slots: `:id`, `:name`
  - command payload slots: `:session-id`, `:session-name`
- `/tree` completion candidates now include `:worktree-path`, so sibling sessions remain distinguishable even when unnamed.
- Regression coverage added in `components/emacs-ui/test/psi-test.el` for:
  - command-payload key support in session display names
  - worktree-aware `/tree` candidate labels
- Verification:
  - `bb emacs:test`
...
- ✓ `clojure-test` CI failure fixed (commit `1d9b648`): `footer-updated-payload` test was hardcoding `/Users/duncan/...` as cwd; tilde-shortening produced the wrong result on CI where `user.home` is `/home/runner`. Fixed by deriving cwd from `(System/getProperty "user.home")` so the assertion is portable. All three CI jobs now pass.
...
- PSL follow-up (commit `6d9a07c`): session-switch transcript rehydration now renders user messages correctly. Root cause: backend serialises `:role` as string `"user"`; `(intern "user")` → bare symbol `user`, not keyword `:user`, so `(eq role :user)` always failed and every replayed message rendered as `ψ:`. Fix: `psi-emacs--role-is-user-p` helper accepts `"user"`, `'user`, and `:user`; used in both render sites. Regression ERT coverage added; 209/209 passing.
