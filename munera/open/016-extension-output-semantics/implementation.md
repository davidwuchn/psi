...
- Nullable extension test helpers were updated to understand the explicit `psi.extension/notify` and `psi.extension/append-message` mutations.
- `extensions.work-on` was already migrated to `:notify`; tests now no longer carry a dead local stdout-capture helper.
- The legacy `send-message` compatibility layer has now been fully removed from runtime, mutation, test-helper, and docs surfaces.
...
- background-job terminal emission now uses explicit runtime notify semantics
- docs/tests were first updated to mark `send-message` as compatibility-only and then the compatibility surface itself was removed
