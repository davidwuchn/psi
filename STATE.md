...
- ✓ Emacs Anthropic `/new` rehydrate flow now consumes canonical agent-history payloads end-to-end (2026-03-16, commit `98fff62`): RPC `session/rehydrated` events for `new_session` / `/new` now emit canonical agent messages (`:role` + `:content`) instead of TUI display rows (`:role` + `:text`), so Emacs rebuilds the transcript from the same message shape the executor/provider path expects. Focused RPC regressions now lock canonical rehydrate payloads for `new_session` and `/resume`; Emacs regressions now lock both canonical session replay and the `/new` clear+rebuild lifecycle so the temporary local echo / command-result path does not leave duplicate assistant turns behind after `session/resumed` + `session/rehydrated`. `bb emacs:test` now explicitly loads those new regression files, and verification is green at 222/222.
- ✓ Anthropic prompt caching now advertises the required beta and preserves provider-side diagnostics (2026-03-16, commit `5f6ec96`): `components/ai/src/psi/ai/providers/anthropic.clj` now adds the `prompt-caching-2024-07-31` beta whenever emitted request content carries Anthropic `cache_control`, composing it with existing OAuth and interleaved-thinking betas instead of forcing callers to choose one beta surface. The same change set also replaces the prior generic Anthropic error collapse with body-aware parsing that preserves provider error text and appends HTTP status plus `request-id` when present, so TUI/CLI failures surface actionable diagnostics instead of bare `Error`. Regression coverage in `components/ai/test/psi/ai/providers/anthropic_test.clj` now locks both prompt-caching beta emission and rich Anthropic error text. Verification green via `clj-kondo --lint components/ai/src/psi/ai/providers/anthropic.clj components/ai/test/psi/ai/providers/anthropic_test.clj` and `bb clojure:test:unit --focus psi.ai.providers.anthropic-test`.
- ✓ Explicit Emacs `/resume <path>` and `/tree <id>` now clear stale transcript state through canonical session rehydrate events (2026-03-16, commit `7383729`): RPC `command` handling now treats `/resume <path>` and `/tree <session-id>` as first-class session transitions, emitting `session/resumed` + `session/rehydrated` before the usual `session/updated` / `footer/updated` / `context/updated` snapshots. That brings explicit resume/tree command forms into the same clear+replay lifecycle as `/new`, bare `/resume`, and bare `/tree`, fixing the Emacs symptom where history appeared not to clear or redisplay on direct session-targeting commands and widget actions. The same change set also makes TUI `/tree` switches request a hard redraw so screen clearing matches `/new`. Regression coverage now locks explicit resume/tree rehydrate behavior in RPC and Emacs, plus TUI redraw-on-tree-switch behavior. Verification green via `bb clojure:test:unit --focus psi.agent-session.rpc-test` and `bb emacs:test`.
- ✓ Anthropic replay investigation now has direct regression coverage for the observed failure shape (2026-03-16, commit `ac0efb5`): `components/ai/test/psi/ai/providers/anthropic_test.clj` now locks the structural request shape that combines prior `tool_use`/`tool_result`, extended Anthropic `:thinking`, and Anthropic `cache_control`, while `components/agent-session/test/psi/agent_session/executor_test.clj` now proves that prior OpenAI-style `:thinking-delta` remains transient and is not replayed into later Anthropic requests. Current evidence therefore points away from conversation-history leakage and toward an intermittent Anthropic-side failure when current-request thinking is enabled alongside prior tool history.
- ✓ Anthropic system prompt caching now excludes volatile runtime metadata (2026-03-16, commit `f28c93f`): `components/agent-session/src/psi/agent_session/system_prompt.clj` now treats the date/time + cwd/worktree footer as a separable runtime tail appended after extension prompt contributions, and `components/agent-session/src/psi/agent_session/executor.clj` projects Anthropic system prompt blocks so only the stable prompt body receives `{:cache-control {:type :ephemeral}}` while the runtime tail remains uncached. This preserves visible runtime metadata ordering without poisoning Anthropic prompt-cache reuse on rebuilt prompts. Regression coverage in `system_prompt_test.clj`, `executor_test.clj`, and `core_test.clj` locks the split/block ordering behavior; focused core verification and lint are green.
- ✓ Anthropic provider request telemetry now matches OpenAI capture visibility (2026-03-16, commit `d4d98ba`): `components/ai/src/psi/ai/providers/anthropic.clj` now emits `:on-provider-request` and `:on-provider-response` callbacks with redacted auth headers, tagged as `:provider :anthropic` / `:api :anthropic-messages`, so live session introspection can inspect Anthropic wire requests and SSE events just like OpenAI. Regression coverage in `components/ai/test/psi/ai/providers/anthropic_test.clj` proves both request and response captures fire, and focused Anthropic provider verification plus lint are green.
- ✓ Anthropic prompt caching now flows from session policy to provider wire shape (2026-03-16, commit `0c57fcf`): session state carries `:cache-breakpoints` (default `#{:system}`), executor conversation reconstruction projects that policy into `:system-prompt-blocks` and tool `:cache-control`, and `components/ai/src/psi/ai/providers/anthropic.clj` emits Anthropic `cache_control` only for supported directives (`{:type :ephemeral}`). Shared conversation/schema/spec layers were synchronized around block-form system prompts so provider request metadata can be attached without losing the flat `:system-prompt` view. Full verification green via `bb test`.
- ✓ Footer usage projection now labels cache tokens explicitly as `CR` / `CW` (2026-03-16, commit `f8336d4`): the RPC footer stats line in `components/agent-session/src/psi/agent_session/rpc.clj` now renders cache-read/cache-write token totals with two-letter labels instead of ambiguous single-letter `R` / `W`. Regression coverage in `components/agent-session/test/psi/agent_session/rpc_test.clj` proves both labels render, and verification is green via `bb clojure:test:unit` plus `bb emacs:test`.
...
- ✓ Emacs session rehydrate lifecycle now clears stale buffer content for `/new`, `/tree`, and `/resume` (2026-03-16, commit `b3a5769`): Emacs now subscribes to and handles canonical `session/resumed` + `session/rehydrated` events by resetting transcript/render state immediately and replaying session messages from the rehydrate payload, instead of depending on ad hoc fetch/rebuild paths alone. The backend `command` path for `/new` now also emits those canonical rehydrate events and normalizes its command-result type to `new_session`, bringing `/new` into the same clear+rehydrate lifecycle as session switching flows. Regression coverage now locks frontend topic subscription, transcript reset on `session/resumed`, replay on `session/rehydrated`, and backend `/new` command emission. Verification green via `bb emacs:test` (218/218), `bb emacs:e2e`, and `bb clojure:test:unit` (852 tests / 4828 assertions).
- ✓ Emacs `/tree` now shows compact session age derived from canonical session timestamps (2026-03-16, commit `51b669f`): backend context session projections and RPC `context/updated` payloads now include `:created-at`, sourced from stable session creation metadata in the runtime context index. Emacs renders those raw timestamps into compact relative labels (`now`, `53m`, `2h`, ...) and appends them alongside the session name/fallback and worktree path in both the session-tree widget and `/tree` completion candidates. Regression coverage now locks timestamp propagation in core/resolver/RPC suites and compact-age rendering in Emacs; verification green via `bb emacs:test` (215/215) and `clojure -M:test --focus unit --skip-meta integration --fail-fast` (849 tests / 4821 assertions).
- ✓ Emacs `/tree` selector labels now distinguish sessions across both event and backend-command payload shapes (2026-03-16, commit `a210c7c`): `psi-emacs--session-display-name` now supports both `:id`/`:name` and `:session-id`/`:session-name`, fixing the blank `(session )` fallback when `/tree` is populated from backend command/frontend-action payloads. `/tree` candidates now also include `:worktree-path`, so unnamed sibling sessions remain distinguishable by repo path. Regression coverage added in `components/emacs-ui/test/psi-test.el`; verification green via `bb emacs:test` (213/213).
- ✓ PSL follow-up for `1bd1f17` converged Emacs slash-command routing across plan/state/spec/docs/tests (2026-03-16): slash-prefixed compose input now routes to backend `command` in both idle and streaming states, while non-slash streaming input continues to use `prompt_while_streaming` steer/queue behavior. `spec/emacs-frontend.allium` and `spec/prompt-slash-commands.allium` now encode slash-first routing explicitly; Emacs docs no longer frame `/...` as an idle-only concept; helper/test vocabulary was renamed away from `idle-slash-*`; `/tree` preserves backend fallback when no local context snapshot exists. Verification green: `allium check spec/emacs-frontend.allium spec/prompt-slash-commands.allium`, `bb emacs:test` (211/211).
...
- ✓ `/work-done` now replaces `/work-merge` as the worktree completion workflow (2026-03-16, commit `7757920`): the extension now caches the default branch on init/session switch, checks whether the current linked branch already contains the default-branch HEAD, auto-rebases through a forked sync subagent when needed, then fast-forward merges from the main worktree context and only afterwards removes the worktree and deletes the branch. `spec/work-on-extension.allium`, `spec/git-worktrees.allium`, `doc/tui.md`, `doc/emacs-ui.md`, and `extensions/test/extensions/work_on_test.clj` are synchronized to the `/work-done` flow; query surfaces now expose the default branch as `:git.branch/default-branch`. Focused extension verification green at 71 tests / 339 assertions; broader unit verification green at 852 tests / 5219 assertions; lint clean.
- ✓ PSL follow-up for `d7e2e28` converged worktree/default-branch plan/state memory to the direct-root query surface (2026-03-16): repo memory now treats `:git.worktree/*` and `:git.branch/default-branch` as the steady-state session-root attrs for worktree orchestration, rather than describing the removed `:psi.agent-session/git-*` compatibility layer. `PLAN.md` Step 15f, `STATE.md`, `spec/git-worktrees.allium`, `spec/work-on-extension.allium`, resolver/introspection tests, `/status`, and the `/work-on` extension all describe the same direct-root shape. Verification remains green: `bb clojure:test:extensions`, `bb clojure:test:unit`, `bb lint`.
- ✓ Current Clojure unit-suite baseline after the introspection surface repair is green: `852 tests`, `4808 assertions`, `0 failures` via `bb clojure:test:unit`.
...
- ✓ Startup context bootstrap now creates exactly one real session (2026-03-14, commit `87a5e77`): `create-context` starts with an empty context session index, `bootstrap-in!` no longer creates a session branch, and `bootstrap-runtime-session!` now creates one real session then runs startup prompts inside that same session. Startup context snapshots no longer include phantom seed/bootstrap/startup artifacts. Focused verification green at 86 tests / 727 assertions across main/core/resolvers/introspection suites.
...
- ✓ RPC handshake now emits `context/updated` bootstrap snapshot for rpc-edn sessions (2026-03-13, commit `a639f3e`) via `:handshake-host-updated-payload-fn` + handshake event emission path in `psi.agent-session.rpc`.
- ✓ Emacs `/tree` session selection is simplified to a single source of truth (`context-snapshot`) with no `list_sessions` fallback branch (commit `a639f3e`); startup context state is expected from handshake bootstrap + subscribe lifecycle.
- ✓ Handshake coverage now includes context snapshot event assertion in `components/agent-session/test/psi/agent_session/rpc_test.clj` (commit `a639f3e`), preserving protocol-level initialization guarantees for session-tree consumers.
...
- ✓ Multi-session host non-UI foundation landed (2026-03-13, commit `b1fef75`): new in-process context session index (`context_index.clj`) tracks `:sessions` + `:active-session-id`; core context now carries `:context-index-atom` and auto-upserts host metadata on session state changes.
- ✓ Core session targeting primitives added: `ensure-session-loaded-in!` (load/switch by session id from context session index) and `set-context-active-session-in!` (routing pointer update) in `components/agent-session/src/psi/agent_session/core.clj`.
- ✓ RPC session routing surface expanded: targetable ops now accept optional `:session-id` and pre-load target session; `switch_session` now accepts `:session-id` (alongside legacy `:session-path`); `list_sessions` op returns context snapshot (`active-session-id` + sessions).
...
- ✓ Multi-session UI spec landed (2026-03-13, commit `62f46cd`): `session-management.allium` defines `ContextSessionSnapshot`/`SessionSlot` and `context/updated` event emission rules; `emacs-frontend.allium` defines session tree widget (`psi-session/session-tree`, left placement), display name fallback, parent-child indent, `/tree` completing-read picker, switch dispatch, connecting affordances, and transcript rehydrate contracts; `EmacsFrontendSessionTreeApi` surface defers full browser to later.
- ✓ Multi-session UI implemented (2026-03-13, commit `16ce8ed`): `context/updated` RPC event emitted on subscribe + `new_session` + `switch_session`; `psi-session/session-tree` left-panel widget rendered from event (hidden when ≤1 session, active marker, streaming badge, parent-child indent, clickable `/tree <id>` actions); `/tree` slash command opens completing-read picker or dispatches direct switch by id; `context-snapshot` stored on Emacs state; 204/204 ERT passing.
...
- ✓ Multi-session UI fork gap closed (2026-03-13, commit `7ab1277`): `fork` RPC op emits `context/updated`; subagent creation correctly excluded (isolated context, not a context peer); subscribe/fork/new_session `context/updated` coverage added to rpc_test.
- ✓ TUI multi-session surface landed (2026-03-13, commit `92fc518`): `/tree` now opens a context-backed session picker in TUI (live `:psi.agent-session/context-sessions` + `:psi.agent-session/host-active-session-id`), supports direct `/tree <session-id|prefix>` switching, renders active/streaming/parent-child cues, and routes switching through `session/ensure-session-loaded-in!` rehydrate callback. `/tree` is runtime-gated to TUI (`supports-session-tree?`) with deterministic console/RPC fallback text.
- ✓ TUI `/tree` hierarchy + alignment polish landed (2026-03-13, commit `3c1c385`): context sessions are rendered in explicit parent/child tree order (stable sibling order), rows now carry tree glyphs (`●`, `├─`, `└─`, `│`), and right-side status cells are column-aligned for mixed `[active]`/`[stream]` states with stable session-id suffix alignment.
...
- ✓ Changelog memory is synced for `/tree` rollout (2026-03-13, commit `d869843`): `CHANGELOG.md` now records `/tree` command modes, TUI-only gating, context-backed selector semantics, switch callback routing, and cross-suite verification totals for the shipped multi-session TUI slice.
...
- ✓ Host-index EQL exposure hardened with process+persisted coverage (2026-03-13): `components/agent-session/test/psi/agent_session/resolvers_test.clj` now validates `:psi.agent-session/context-*` process view, `:psi.session/list` persisted view, and graph-introspection discoverability across `:psi.graph/root-queryable-attrs` and `:psi.graph/edges`.
- ✓ Multi-session regression suite is green after context-session hardening (2026-03-13): `clojure -M:test --focus psi.agent-session.rpc-test` (32 tests, 316 assertions) and `clojure -M:test --focus psi.agent-session.resolvers-test` (32 tests, 304 assertions) both pass with 0 failures.
- ✓ Graph contract nuance captured in tests: `:psi.agent-session/context-sessions` is join-discoverable via `:psi.graph/edges`; root-queryable scalar attr listings are not required to include join attrs.
- ✓ Commit `b1fef75` consolidated the session-work sweep: runtime context routing, session persistence/header/schema convergence, startup spawn-policy alignment, resolver/RPC/session tests, and related Allium/UI contract updates now land together as one verified delta.
...
- ✓ Step 11a git-worktree visibility (read-only) implemented: `:git.worktree/*` attrs, session-root queryability, `/worktree` command, and `/status` worktree surfacing.
