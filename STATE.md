...
- ✓ Context/session-tree Allium model is now the leading architecture description (2026-03-15, commit `b935191`): the spec set now treats context as the owner of shared resources (tools, prompt templates, skills, extensions) and sessions as the unit of conversation, execution, and lineage within that context. Subagents are modeled as child sessions, startup prompts are framed as context/session bootstrap, and frontend/session-selection surfaces now point toward context-scoped session focus rather than host-registry semantics. `allium check` is green across the converged spec slice, but code/tests still retain legacy runtime vocabulary in places (`host/updated`, host registry, single-active-session machinery), so the spec is ahead of the implementation.
