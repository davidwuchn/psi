...
- ✓ `/work-done` now replaces `/work-merge` as the worktree completion workflow (2026-03-16): the extension now caches the default branch on init/session switch, checks whether the current linked branch already contains the default-branch HEAD, auto-rebases through a forked sync subagent when needed, then fast-forward merges from the main worktree context and only afterwards removes the worktree and deletes the branch. `spec/work-on-extension.allium`, `doc/tui.md`, `doc/emacs-ui.md`, and `extensions/test/extensions/work_on_test.clj` are synchronized to the `/work-done` flow; a new query surface now exposes the default branch as `:git.branch/default-branch` and `:psi.agent-session/git-default-branch`. Focused extension verification green at 71 tests / 339 assertions; broader unit verification green at 852 tests / 4816 assertions; lint clean.
