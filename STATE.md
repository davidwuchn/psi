...
- ✓ Worktree/default-branch query surface is now simpler (2026-03-16): runtime consumers query `:git.worktree/*` and `:git.branch/default-branch` directly from session root; the redundant `:psi.agent-session/git-*` bridge attrs and resolvers were removed. `spec/git-worktrees.allium`, `spec/work-on-extension.allium`, resolver/introspection tests, `/status`, and the `/work-on` extension are synchronized to the direct-root shape. Verification green: `bb clojure:test:extensions`, `bb clojure:test:unit`, `bb lint`.
