...
- ✓ pre-commit `clj-kondo-lint` hook is active (2026-03-15, commit `accb233`): `clj-kondo --cache false --lint` runs on every staged Clojure file before commit; any warning or error blocks the commit. Root `.clj-kondo/config.edn` is now the canonical source of macro-alias hints for individual-file linting (Pathom3 `defresolver`/`defmutation`, Guardrails `>defn`, Malli, Promesa, Potemkin). `doc/develop.md` updated.
- ✗ `bb test` remains an outstanding convergence item for CI green.
