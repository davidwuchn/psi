...
- ✓ Subagent optional fork capability implemented (2026-03-12): `subagent(action=create)` accepts `fork_session` (default false) to inherit invoking-session conversation context; `/sub` now supports `--fork|-f`; subagent widget/list annotate forked runs; extension spec updated in `spec/subagent-widget-extension.allium`.
