...
- ✓ Tri-artifact agreement invariant added to AGENTS.md (commit `c98e310`): `λ(spec, tests, code) → ∀ artifact ∈ {spec, tests, code}, ∀ change δ: propagate(δ) → remaining two such that agree(spec, tests, code) = true at all times`. Any change to one artifact must propagate to the other two.
