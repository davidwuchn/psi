...
- ✓ Immediate verification state after wiring CI is not yet green: `bb fmt:check` initially failed on incorrect cljfmt task invocation, `bb lint` exposed a large pre-existing repo-wide backlog (`237 errors`, `148 warnings`, plus a missing `test` path), and `bb test` failed early because `psi.memory.datalevin_test` still loads after `psi.memory.datalevin` removal. CI exists as the canonical verification surface, but the repository still needs convergence before that surface can pass on GitHub.
