...
- ✓ Conversation domain now has a converged code/spec/test slice (2026-03-15, commit `14ba6de`): `psi.ai.conversation` was simplified around shared append/update helpers, unused public functions were removed, conversation-specific tests were split into `components/ai/test/psi/ai/conversation_test.clj`, and `spec/conversation.allium` now captures the concrete embedded conversation behavior surface. The spec is validated with `allium check`, and focused conversation/core tests are green.
