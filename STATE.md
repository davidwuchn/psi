...
- ✗ `bb test` green on CI remains outstanding — one pre-existing `clojure-test` failure: `footer payload mirrors...` asserts a hardcoded tilde-shortened local path that does not match CI runner paths.
