...
- ✓ Emacs slash-command routing is now run-state independent (2026-03-16): slash-prefixed compose input now routes to backend `command` in both idle and streaming states, while non-slash streaming input continues to use `prompt_while_streaming` steer/queue behavior. `spec/emacs-frontend.allium` and `spec/prompt-slash-commands.allium` now encode slash-first routing explicitly; Emacs docs no longer frame `/...` as an idle-only concept; ERT coverage includes streaming slash-command dispatch plus preserved non-slash streaming behavior. Verification green: `allium check spec/emacs-frontend.allium spec/prompt-slash-commands.allium`, `bb emacs:test` (211/211).
