...
(ert-deftest psi-background-jobs-projection-renders-widget-and-statuses ()
  (with-temp-buffer
    ...
    (psi-emacs--handle-rpc-event
     '((:event . "ui/widgets-updated")
       (:data . ((:widgets . [((:placement . "below-editor")
                               ...
                               (:content-lines . [((:text . "job-1  [running]  delegate")
                                                   (:action . ((:type . "command") (:command . "/job job-1"))))
                                                  ((:text . "job-2  [pending-cancel]  delegate")
    ...
    (let ((buf (buffer-string)))
      (should (string-match-p (regexp-quote "job-1  [running]  delegate") buf))
      (should (string-match-p (regexp-quote "job-2  [pending-cancel]  delegate") buf))
