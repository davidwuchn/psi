...
(defn handle-new-session!
  [{:keys [ctx request state session-id on-new-session!]}]
  (let [source-session-id (or (events/focus-session-id state)
                              session-id)
        ...
        response-data     (emit/emit-navigation-result! emit! ctx state nav)]
